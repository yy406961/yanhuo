<template>
    <div class="graphBox">
        ...
        <div class="introduce" >
            ...
            <button @click="thresholdConfig"
                    v-if="options.type===1 && categoryName.length"
                    class="el-button el-button--primary"
                    style="margin: 0 14px;">阈值设置</button>
        </div>
        <div class="screen" v-if="outerTypeflag">
            <div class="screenText">通联人数：</div>
            <div class="select">
                <el-select v-model="value" placeholder="请选择" @change="selectChange">
                    <el-option
                        v-for="item in outerLimit"
                        :key="item.value"
                        :label="item.name"
                        :value="item.value">
                    </el-option>
        ...
        <sysDialog size="tiny"
                    title="阈值设置"
                    @change="dialogChangeThreshold"
                    @openCallback="dialogOpendCallbackThreshold"
                    :dialogVisible="dialogVisibleThreshold"
                    :okFn="okFnThreshold">
                    <el-row>通联次数范围：</el-row>
                    <el-row>
                        <el-col :span="2">&nbsp;</el-col>
                        <el-col :span="2">低：</el-col>
                        <span class="innerStart">0 - </span>
                        <el-col :span="6">
                            <el-input v-model="innerThreshold.innerLowEnd"></el-input>
                        </el-col>
                    </el-row>
                    <el-row>
                        <el-col :span="2">&nbsp;</el-col>
                        <el-col :span="2">中：</el-col>
                        <span class="innerStart">{{ Number(innerThreshold.innerLowEnd) + 1 }} - </span>
                        <el-col :span="6">
                            <el-input v-model="innerThreshold.innerMiddleEnd"></el-input>
                        </el-col>
                    </el-row>
                    <el-row>
                        <el-col :span="2">&nbsp;</el-col>
                        <el-col :span="2">高：</el-col>
                        <span class="innerStart">{{ Number(innerThreshold.innerMiddleEnd) + 1 }} - </span>
                        <el-col :span="6">
                            <el-input v-model="innerThreshold.innerHighEnd"></el-input>
                        </el-col>
                    </el-row>
        </sysDialog>
    </div>
...
export default {
    data() {
        return {
            ...
            // 外联时候的VIEW 图例集合
            outerCategoryName: [
                { name: '内部', color: COLORS.H },
                { name: '外部', color: COLORS.L }
            ],
            // 界面图例集合
            categoryName: [],
    ...
    methods: {
        ...
        okFnThreshold() {
            let re = /^[1-9]+[0-9]*]*$/
            if (
                isNaN(this.innerThreshold.innerLowEnd) ||
                !re.test(this.innerThreshold.innerLowEnd) ||
                isNaN(this.innerThreshold.innerMiddleEnd) ||
                !re.test(this.innerThreshold.innerMiddleEnd) ||
                isNaN(this.innerThreshold.innerHighEnd) ||
                !re.test(this.innerThreshold.innerHighEnd)
            ) {
                this.$message.error('请输入正整数')
            } else if (
                Number(this.innerThreshold.innerLowEnd) + 1 >= this.innerThreshold.innerMiddleEnd ||
                Number(this.innerThreshold.innerMiddleEnd) + 1 >=
                    this.innerThreshold.innerHighEnd ||
        ...
        initPersonnelNode() {
            ...
            this.phoneData.forEach(item => {
                ...
                if (item.type === DATA_TYPE.IN) {
                    const categoryObj = me.innerCategoryLimit(item.count)
                    // 是外部和组件 内部统一颜色
                    const _color = this.options.type === DATA_TYPE.IN ? categoryObj.color : COLORS.H
                    const _colorB =
                        this.options.type === DATA_TYPE.IN ? categoryObj.colorB : COLORS.HB
                    ...
                    if (this[`outerNodes${categoryObj.index}`] instanceof Array) {
                        this[`outerNodes${categoryObj.index}`].push(node)
                    }
                }
            })
        },
        initData() {
            ...
            //                console.log(this.insidePersonnel)
                ...
                tooltip: {
                    ...
                    formatter: function(res) {
                        ...
                        if (res.data.Original.cname !== undefined) {
                            content = `姓名：${res.data.Original.cname}<br/>`
                        }
                        if (me.options.type === 1) {
                            content += `手机号：${res.name}<br/>通联次数：${res.value}`
                        } else {
                            content += `手机号：${res.name}<br/>通联人数：${res.value}`
                        }
                        return content
                    }
                },
        ...
        startGLrender() {
            let { nodes, links, categories } = this.option.series[0]
