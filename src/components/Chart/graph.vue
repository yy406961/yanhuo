    ...
    export default {
        ...
        watch: {
            options: {
                ...
                handler (val) {
                    ...
                    // 恢复阈值初始值
                    if (val.resetNum) {
                        this.innerThreshold.innerLowEnd = 1;
                        this.innerThreshold.innerMiddleEnd = 3;
                    }
        ...
        methods: {
            ...
            okFnThreshold () {
                ...
                } else if (((Number(this.innerThreshold.innerLowEnd) + 1) >= this.innerThreshold.innerMiddleEnd) || this.innerThreshold.innerLowEnd <= 0
                    ) {
            ...
            initPersonnelNode () {
                ...
                this.phoneData.forEach(item => {
                    ...
                    if (item.type === DATA_TYPE.IN) {
                        ...
                        const _colorB = this.options.type === DATA_TYPE.IN ? categoryObj.colorB : COLORS.HB
                    ...
                    tooltip: {
                        ...
                        formatter: function (res) {
                            ...
                            if (res.data.Original.cname !== undefined) {
                                let name = me.isPhoneJM ? phoneNumChange(res.data.Original.cname) : res.data.Original.cname
                                content = `姓名：${name}<br/>`;
                            }
                            if (me.options.type === 1) {
                                let name = me.isPhoneJM ? phoneNumChange(res.name) : res.name
                                content += `手机号：${name}<br/>通联次数：${res.value}`
                            } else {
                                let name = me.isPhoneJM ? phoneNumChange(res.name) : res.name
                                content += `手机号：${name}<br/>通联人数：${res.value}`
                            }
                            return content
                        }
                    },
                            ...
                            normal: {
                                ...
                                formatter: function(params) {
                                    ...
                                    if (params.data.Original.number) {
                                        ...
                                        return params.data.Original.inNumber
                                    }
                                }
                            }
                        },
