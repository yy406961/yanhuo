<template>
    <div class="ga-mapRegin">
        <formList :option="formOpt" ref="form"></formList>
        <el-row :span="24">
            <el-col :span="24">
                <gis :style="mapHeight" @mapCoordinate="getMapCoordinate" ref="gisMap" :option="mapData"></gis>
            </el-col>
        </el-row>
        <div class="mapCenterBtn">
            <buttonComp :option="mapSaveBtnOpt" v-show="mapRangeBtn"></buttonComp>
            <buttonComp :option="mapReviseBtnOpt" v-show="!mapRangeBtn"></buttonComp>
            <!--<buttonComp :option="mapCancelBtnOpt" v-show="!mapRangeBtn"></buttonComp>-->
        </div>
    </div>
</template>
<script>
...
import gis from '../taskCreate/config/mapAreaUtil'
export default {
    name: '',
    data() {
        const me = this
    ...
    methods: {
        ...
        judgeIsRight() {
            ...
            if (!areaName) {
                ...
                this.$message.warning('只能框选一个区域，请重新框选')
                this.mapCoordinate = []
            } else {
                isRight = true
                let list = []
    ...
    computed: {
        ...
        mapHeight() {
            return {
                height: this.visualHeight - this.formHeight - 76 + 'px',
                width: '100%'
            }
        }
    },
    mounted() {
        ...
        if (this.$route.params.list) {
            ...
            this.cellInfo = this.$route.params.list.cellInfo
            ...
            localStorage.setItem('cellInfo', JSON.stringify(this.$route.params.list.cellInfo))
        }
        this.$refs.form.setValue([{ id: 'areaName', value: this.areaName }])
        // 刷新页面
