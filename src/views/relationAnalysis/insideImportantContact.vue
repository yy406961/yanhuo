                ...
                <el-col :span="7">
                    <div id="leftTable">
                        <frameComp :normal="true">
                            <tableComp :option = "tableOptResult"
                            :comSize="comSizeComm"
                            ref="tableResult"
                            ></tableComp>
                        </frameComp>
                    ...
                    <div class="ga-outImportantContact" :style="rightStyle">
                        <div class="ga-outImportantContact__chart" id="graphBox">
                            <div :style="rightStyle">
                                <graphComp :options="opts" @graphClick="clickHandler" :size="rightStyle" ref="chart"></graphComp>
                            </div>
                        </div>
                        <transition name="fade">
                            <div class="ga-outImportantContact__minTable" v-show="minTableShow" ref="minTable">
                                ...
                                <div class="ga-outImportantContact__tableContent">
                                    <frameComp :normal="true">
                                        <tableComp :option = "tableOptMin" :comSize="comSizeComm"></tableComp>
                                    </frameComp>
                                </div>
                            </div>
                        </transition>
                        <transition name="fade">
                            <div class="ga-outImportantContact__table" v-show="tableShow" ref="detailTable">
                                ...
                                <div class="ga-outImportantContact__tableContent">
                                    <frameComp :normal="true">
                                        <tableComp
                                            :option = "tableOptCommDetail"
                                            :comSize="comSizeComm"
                                            @handleCurrentPageChange="commDetailCurrentPageChange"
                                            @handleSizePageChange = "commDetailSizePageChange"
                                        ></tableComp>
                                    </frameComp>
                                </div>
                            </div>
...
export default {
        ...
        return {
            ...
            tableOptResult: {
                ...
                column: [
                    ...
                    { name: '姓名', value: 'cname', width: 53, align: 'center', tooltip: true },
                    { name: '内部号码', value: 'relateNum', width: 120, align: 'center' },
                    { name: '归属地', value: 'homeName', align: 'center' },
                    { name: '人数', value: 'connPersonCount', width: 40, align: 'center' },
                    { name: '次数', value: 'amount', width: 40, align: 'center' }
                ],
                data: [],
                rowClick: function(row) {
                    me.tableShow = false
                    me.tableOptCommDetail.listLoading = false
                    me.insideNumber = row.relateNum
                    let minMsgPass
                    if (me.isPhoneJM) {
                        minMsgPass = phoneNumChange(me.insideNumber)
                    } else {
                        minMsgPass = me.insideNumber
                    }
                    me.minMsg = `${minMsgPass}与内部人员通联分析`
                    me.outInContactsList(me.insideNumber, me.taskID)
                    me.showMinTable()
            ...
            tableOptMin: {
                ...
                column: [
                    ...
                    { name: '号码', value: 'innerMsisdn', align: 'center' },
                ...
                rowClick: function(row) {
                    ...
                    let insideNumberPass, numDetailPass
                    if (me.isPhoneJM) {
                        insideNumberPass = phoneNumChange(me.insideNumber)
                        numDetailPass = phoneNumChange(me.numDetail)
                    } else {
                        insideNumberPass = me.insideNumber
                        numDetailPass = me.numDetail
                    }
                    me.commDetailMsg = insideNumberPass + '/' + numDetailPass + ' 通联详情'
                    me.tableOptCommDetail.pageOpt.pageSize = 10
                    me.tableOptCommDetail.pageOpt.currentPage = 1
                    me.outInContactsInfo(me.insideNumber, me.numDetail, 1, 10)
                    me.showDetailTable()
                }
            },
            // 中间表格：通联详情
    ...
    methods: {
        ...
        handleDownload () {
            let name = '内部重点人分析'
            let inHeader = ['序号', '姓名', '内部号码', '归属地', '人数', '次数']
            let inFilterVal = ['index', 'cname', 'relateNum', 'homeName', 'connPersonCount', 'amount']
            let contactHeader = ['序号', '姓名', '号码', '通联次数', '归属地']
            let contactFilterVal = ['inNumber', 'cname', 'innerMsisdn', 'amount', 'homeName']
            if (this.minTableShow) {
                let data = []
                data.push(this.tableOptResult.data)
                data.push(this.tableOptMin.data)
                this.exportExcel(
                    [inHeader, contactHeader],
                    [inFilterVal, contactFilterVal],
                    data,
                    ['内部号码', this.insideNumber + '通联分析'],
                    name
                )
            } else {
                let data = []
        ...
        imgScreenshot () {
            html2canvas(document.getElementById('graphBox'), {
                ...
                onrendered: function(canvas) {
                    ...
                    // 下载下来的图片名字
                    $('#screenshot').attr('download', '内部重点人关系图.jpg');
        ...
        // 中间表格分页
        commDetailCurrentPageChange(val) {
            this.tableOptCommDetail.pageOpt.currentPage = val
            let pageSize = this.tableOptCommDetail.pageOpt.pageSize
            this.outInContactsInfo(this.insideNumber, this.numDetail, val, pageSize)
        },
        commDetailSizePageChange(val) {
            this.tableOptCommDetail.pageOpt.currentPage = 1
            this.tableOptCommDetail.pageOpt.pageSize = val
            if (this.tableOptCommDetail.data.length !== 0) {
                this.outInContactsInfo(this.insideNumber, this.numDetail, 1, val)
            }
        },
        // 添加查询任务dialog显示回调
        ...
        async contactInfoList(taskID) {
            ...
            if (rows) {
                ...
                this.opts.resetNum = true
                // 给右边图表中的点加上编号，同左边表格想匹配
                rowsFilter.forEach((item, index) => {
        ...
        // 右侧表格：获取外部与内部通联列表
        async outInContactsList(num, taskId) {
        ...
        clickHandler(arg) {
            if (arg) {
                ...
                let jump = a * 30
                $('#leftTable .el-table__body-wrapper').animate({ scrollTop: jump }, 500)
                this.insideNumber = arg.name
                let insideNumberPass
                let curRow = this.$refs.tableResult.tableListData[a]
                if (this.isPhoneJM) {
                    insideNumberPass = phoneNumChange(this.insideNumber)
                } else {
                    insideNumberPass = this.insideNumber
                }
                this.$refs.tableResult.setRow(curRow)
                this.minMsg = `${insideNumberPass}与内部人员通联分析`
                this.showMinTable()
                this.outInContactsList(this.insideNumber, this.taskID)
            }
        },
        // 右侧表格
        showMinTable() {
