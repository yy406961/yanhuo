<template>
    <div style="width: 100%;height: 100%">
        <el-row class="figurePortrait-baseData__base--title smallNotLong">
            ...
            <span class="basetitle">上网情况</span>
                    ...
                    <frameComp>
                        ...
                        <div class="figurePortrait-baseData__frame--content">
                            <div>
                                ...
                                <internetBar v-show="treeMapShow"
                                             :option="treeOpt"
                                             ref="treeRef"
                                             :renderData="treeData" @barToInternetShow="barToInternetShow">
                                </internetBar>
                            </div>
                        </div>
                    </frameComp>
...
<script>
    ...
    // import internetBar from './barInternet'
    import internetBar from 'components/Chart/figurePortrait/newBarInternet'
    // import barComp from './barPeriod'
    import barComp from 'components/Chart/figurePortrait/newLinePeriod'
    import { getInternetUseData } from 'api/relationAnalysis/newFigurePortrait/index'
    export default {
        name: 'appUse',
...
//                },
                ...
                sensitiveOpt: {
                    ...
                    column: [
                        ...
                        { name: '敏感网站', value: 'name', align: 'center' },
                ...
                usualOpt: {
                    ...
                    column: [
                        { name: '序号', value: 'index', width: 60, align: 'center' },
                        { name: '其他网站', value: 'name', align: 'center' },
                        { name: '使用次数', value: 'count', width: 100, align: 'center' },
                        { name: '第一次使用时间', value: 'firstTime', width: 150, align: 'center' },
                        { name: '最后一次使用时间', value: 'lastTime', width: 150, align: 'center' }
                    ],
...
//            },
            'renderData' (val) {
                ...
                this.internetUseTimeObj.msisdn = val.msisdn
                if (this.radioVal === 0) {
                    this.getInternetUseDataFun()
                } else {
                    this.radioVal = 0
                }
                // this.getInternetUseTimeFun();
            },
            'radioVal' (val) {
                if (val === 3) {
                ...
                if (this.renderData) {
                    ...
                    this.internetUseTimeObj.type = val;
                    //  按月选择
                    if (val === 3) {
                        if (!this.chooseMonth) {
                            this.$message({
                        ...
                        this.internetUseTimeObj.time = monthVal
                    } else {
                        this.formObj.time = 'null'
                        this.internetUseTimeObj.time = 'null'
                    }
                    this.getInternetUseDataFun()
                    // this.getInternetUseTimeFun()
                }
            ...
            'chooseMonth' (val) {
                ...
                if (val) {
                    if (this.chooseMonthCount && this.renderData) {
                        ...
                        this.internetUseTimeObj.time = this.chooseMonth
                        this.getInternetUseDataFun();
                        // this.getInternetUseTimeFun()
                    }
                }
        ...
        methods: {
            ...
            getInternetUseDataFun () {
                ...
                getInternetUseData(this.formObj).then(resp => {
                    if (resp && resp.data) {
                        this.reqFinish = true
                        this.sensitiveOpt.data = resp.data.sensitive
                        this.usualOpt.data = resp.data.other
                        this.treeData = resp.data.count
                        this.getInternetUseTimeFun(true, this.treeData)
                        this.cancelLoading(0, this.sensitiveOpt, resp.data.sensitive)
                        this.cancelLoading(0, this.usualOpt, resp.data.other)
                        this.cancelLoading(1, ['treeData', resp.data.count, 'treeMapLoading', 'treeMapShow'])
                    }
                }).catch(e => {
                    if (e.status === 300) {
            ...
            // async getInternetUseTimeFun () {
            //     this.loadLoading(1, ['barData', 'barLoading', 'barShow'])
            //     let resp = await getInternetUseTime(this.internetUseTimeObj)
            //     if (resp && resp.data) {
            //         this.barData = resp.data
            //         this.cancelLoading(1, ['barData', resp.data, 'barLoading', 'barShow'])
            //     }
            // },
            getInternetUseTimeFun (isAll, data) {
                let arr = []
                if (data) {
                    if (isAll) {
                        arr = data
            ...
            barToInternetShow (val) {
                if (val) {
                    this.getInternetUseTimeFun(false, val)
                }
            },
