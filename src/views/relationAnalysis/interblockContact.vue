                ...
                <el-col :span="7">
                    <div id="leftTable">
                        <frameComp :normal="true">
                            <tableComp :option = "tableOptResult" :comSize="comSizeComm" ref="tableResult"></tableComp>
                        </frameComp>
                    </div>
                </el-col>
                <el-col :span="17">
                                ...
                                <div class="ga-outImportantContact__tableContent">
                                    <frameComp :normal="true">
                                        <tableComp :option = "tableOptCommDetail" :comSize="comSizeComm"
                                                   ...
                                                   @handleSizePageChange="commDetailPage"></tableComp>
                                    </frameComp>
                                </div>
                            </div>
                        </transition>
...
//    import { targetList, contactInfoList, contactInfoChart, outInContactsList } from 'api/relationAnalysis/interblockContact'
...
export default {
        ...
        return {
            ...
            tableOptResult: {
                ...
                column: [
                    ...
                    { name: '组间联系人号码', value: 'relateNum', width: 120, align: 'center' },
                    { name: '归属地', value: 'homeName', align: 'center' },
                    { name: '通联人数', value: 'connPersonCount', width: 65, align: 'center' },
                    { name: '通联组数', value: 'groupCount', width: 60, align: 'center' }
                ],
                data: [],
                rowClick: function(row) {
                    me.minTableShowInside = false
                    me.tableOptMinInside.listLoading = false
                    me.tableShow = false
                    me.tableOptCommDetail.listLoading = false
                    me.groupNumber = row.relateNum
                    me.numType = row.type
                    let groupNumberPass
                    if (me.isPhoneJM) {
                        groupNumberPass = phoneNumChange(me.groupNumber)
                    } else {
                        groupNumberPass = me.groupNumber
                    }
                    me.minMsg = `${groupNumberPass}与内部人员通联分析`
                    me.showMinTable()
                    me.outInContactsList(me.groupNumber, me.taskID)
            ...
            tableOptMin: {
                ...
                rowClick: function(row) {
                    ...
                    let groupNumberPass, numDetailPass
                    if (me.isPhoneJM) {
                        groupNumberPass = phoneNumChange(me.groupNumber)
                    ...
                    me.commDetailMsg = groupNumberPass + '/' + numDetailPass + ' 通联详情'
                    me.tableOptCommDetail.pageOpt.currentPage = 1
                    me.tableOptCommDetail.pageOpt.pageSize = me.pageArr
                    if (me.numType === 1) {
                        me.outInContactsInfo(me.groupNumber, me.numDetail, 1, me.pageArr)
                    } else {
                        me.outInContactsInfo(me.numDetail, me.groupNumber, 1, me.pageArr)
                    }
                }
            },
            // 中间表格：组间联系人通联详情
    ...
    methods: {
        ...
        handleDownload () {
            let name = '组间联系人挖掘'
            let outHeader = ['序号', '组间联系人号码', '归属地', '通联人数', '通联组数']
            let outFilterVal = ['index', 'relateNum', 'homeName', 'connPersonCount', 'groupCount']
            let inHeader = ['序号', '内部号码', '归属地', '通联人数', '通联组数']
        ...
        imgScreenshot () {
            html2canvas(document.getElementById('graphBox'), {
                ...
                onrendered: function(canvas) {
                    ...
                    // 下载下来的图片名字
                    $('#screenshot').attr('download', '组间联系人关系图.jpg');
        ...
        commDetailCurrentPage(val) {
            if (this.numType === 1) {
                this.outInContactsInfo(this.groupNumber, this.numDetail, val, this.pageArr)
            } else {
                this.outInContactsInfo(this.numDetail, this.groupNumber, val, this.pageArr)
            }
        },
        commDetailPage(val) {
            ...
            this.pageArr = val
            if (this.numType === 1) {
                this.outInContactsInfo(this.groupNumber, this.numDetail, 1, val)
            } else {
                this.outInContactsInfo(this.numDetail, this.groupNumber, 1, val)
            }
        },
        // 添加查询任务dialog显示回调
        ...
        async contactInfoList(taskID) {
            ...
            if (rows && _.isArray(rows)) {
                ...
                let arr = rowsFilter.filter(item => {
        ...
        clickHandler(arg) {
            if (arg) {
                ...
                this.groupNumber = arg.name
                this.numType = arg.type
                let groupNumberPass
                if (this.isPhoneJM) {
                    groupNumberPass = phoneNumChange(this.groupNumber)
                } else {
                    groupNumberPass = this.groupNumber
                }
                if (arg.type === 1) {
                    this.minMsg = `${groupNumberPass}与组间人员通联分析`
                } else {
                    this.minMsg = `${groupNumberPass}与内部人员通联分析`
                }
                this.showMinTable()
                this.outInContactsList(
                    this.groupNumber,
                    this.taskID,
