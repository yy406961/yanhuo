    ...
    <div class="ga-keyAreaMonitor__contentWrap">
        ...
        <div class="ga-keyAreaMonitor" :style="mainStyle">
            ...
            <div class="ga-keyAreaMonitor__rightWrap">
                <div :class = "index === 4?'active-rightItemWrap':''" class="ga-keyAreaMonitor__rightItemWrap" v-for="(item, index) in rightMapData" :key="index">
            ...
            getCurData() {
                ...
                ajax.post(this.dataUrl2).then(resp => {
                    ...
                    ajax.post('focus/getNum', obj).then(resp => {
                        // 只删掉第一个
                        this.curData.splice(0, 1);
                        let hDdata = resp.data.sszxCount - this.curData[0] - this.curData[2] - this.curData[3];
                        this.curData[1] = hDdata;
                        let curAll = this.curData[0] + this.curData[1] + this.curData[2] + this.curData[3] + this.curData[4];
                        // this.countTitle = '五个区域用户数总和：' + data[7]
                        this.countTitle = '五个区域用户数总和：' + curAll
                    }).catch((err) => {
                        console.log(err)
                    })
                }).catch(err => {
    ...
    @component-namespace ga {
        @b keyAreaMonitor {
            ...
            @e rightItemWrap{
                ...
                /*height: calc((100% - var(--timelineHeight)) * 0.49) ;*/
                /* height:32%; */
                margin-bottom: 3%;
                padding-left:8px;
            }
