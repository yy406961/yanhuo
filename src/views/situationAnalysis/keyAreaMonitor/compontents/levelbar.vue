...
export default {
    ...
    methods: {
        ...
        buildData(data) {
            ...
            let yRightata = []
            let yData = dataArr.map((it, index) => {
                let { length } = dataArr
                ...
                tooltip: {
                    ...
                    formatter(source) {
                        if (source.data.dataObj) {
                            let { tipsText, name, value, flow } = source.data.dataObj
                            let str = `APP名称：${name}<br/> 
                                人数：${value}<br/>
                                流量：${flow}
                                `
                            return tipsText || str
                ...
                series: [
                    {
                        ...
                        itemStyle: {
                            ...
                            color: function(params) {
                                let colorList = params.dataIndex > dataArr.length - 4
                                    ? ['rgba(56,39,176,1)', 'rgba(188,103,212,1)']
                                    : ['rgba(18,54,101,1)', 'rgba(7,164,197,1)']
                                let result = {
                                    type: 'linear',
                                    x: 0,
                                    y: 1,
