...
<script>
    ...
    import { borderData, borderDataA, borderDataB, borderDataC } from './data'
    export default {
        data () {
            return {
                jzShow: true,
                ...
                carLONPoint: [
                    ...
                    { name: '电信-开发银行', value: 3685, lon: '112.9765950689', lat: '28.1901297496' },
        ...
        // 传入参数为点集，以及区域边界点数组
        props: [
            'Interface', 'heatPoint', 'areaBorder', 'gpsMax', 'carData', 'carshow',
            'isWarn0', 'isWarn1', 'isWarn2', 'isWarn3', 'areaId'
        ],
        computed: {
            // 取得地市关键字，用于获取地市坐标
            ...mapGetters({
        ...
        mounted () {
            ...
            this.createMap()
            // if (this.carshow) this.getCarPoint(this.carLONPoint)
        },
        methods: {
            blockRefs(index) {
                return 'Block' + index
            ...
            handelOutArea(data) {
                ...
                if (this.jzShow) {
                    // 橘子洲区域外虚线边框写死
                    munData = borderData
                } else if (this.areaId === 141) {
                    // 韶山景区区域外虚线边框写死
                    munData = borderDataA
                } else if (this.areaId === 142) {
                    // 省信管局区域外虚线边框写死
                    munData = borderDataB
                } else if (this.areaId === 143) {
                    // 湖南省委区域外虚线边框写死
                    munData = borderDataC
                } else {
                    data.forEach(item => {
