...
export default {
        ...
        return {
            ...
            tableOptTaskResult: {
                ...
                column: [
                    ...
                    { name: '目标个数', value: 'personNum', align: 'center', width: 160 },
                        ...
                        operations: [
                            {
                                ...
                                click: e => {
                                    if (e.queryStatus === TASK_STATUS.OK) {
                                        ...
                                        me.parentData.personNum = e.personNum
                                        me.show = false
    ...
    methods: {
        ...
        taskList() {
            ...
            ajax.post(`keyPersonInforSource/selectKeyPersonTaskList`, this.formObj).then(resp => {
                let { rows, total } = resp
                if (rows && rows.length) {
                    rows.map(item => {
                        if (item.queryStatus === TASK_STATUS.SEARCH) {
                            item.statusStr = '查询中'
                        } else if (item.queryStatus === TASK_STATUS.OK) {
                            item.statusStr = '已完成'
        ...
        // shanchu删除
        deleteTask() {
            ajax.post(`keyPersonInforSource/deleteTaskById`, { taskId: this.taskId }).then(resp => {
                if (resp) {
